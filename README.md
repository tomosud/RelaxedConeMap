...
2. AI 深度推定と Cone Map 生成が自動実行されます。進捗は画面下に表示されます。
3. 傾き操作は既定で有効です。端末を傾けると視点が動き、レリーフを覗き込めます。iOS Safari はセンサー権限にユーザー操作が必要なため、プレビューを一度タップした時点で許可を確認します（写真選択と同時に要求するとファイル選択が開けなくなるため）。画面下の **Recenter Tilt** で現在の向きを正面に設定し直せます。
4. 画面下の **Depth** で立体感を調整します。
5. **New Photo** で最初の画面に戻り、別の写真をやり直せます。

モバイルでは実機で完走できるように、次の設定を自動で選びます。

| 項目 | モバイル | PC |
| --- | --- | --- |
| Cone Map 生成方式 | Legacy (Relaxed Cone) 固定 | 既定は Robust RCS (切替可) |
| 解像度 | 256 | 512 |
| Search Radius / Ray Steps | 32 / 16 | 48 / 24 |
| 深度推定モデル | 軽量な `depthanything-quant` を優先 | Depth Anything V3 Small を優先 |
| 深度推定の入力サイズ | 最大 392px | 最大 518px |

Robust RCS は全セルを走査するため生成パスが数千回になり、スマホの GPU では実用的な時間で終わりません。そのためモバイルは GPU Gems 3 方式の Relaxed Cone 生成に固定しています。同じ理由で表示も Relief 固定とし、自己影、スペキュラ、シェーディングの一部を無効にします。PNG 保存、詳細調整、Unreal マテリアル出力は PC 版を使用してください。
